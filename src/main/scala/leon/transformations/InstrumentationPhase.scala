...
abstract class InstrumentationPhase extends TransformationPhase {
  ...
  def instrumentationType : TypeTree 
  ...
  def apply(ctx: LeonContext, program: Program): Program = {
    ...
    val instVar = IsInstrumentationVariable(program)
    program.definedFunctions.foreach((fd) => {
      if (fd.hasPostcondition &&
        exists(instVar)(fd.postcondition.get)) {
    ...
    for (fd <- program.definedFunctions) {      
      if (callees.contains(fd)) {        
        val newRetType = TupleType(Seq(fd.returnType, instrumentationType))
    ...
    def mapBody(body: Expr, f: FunDef) = {
      ...
      if (callees.contains(f)) {
        new ExprInstrumenter(ctx, f, funMap)(body)
      } else {
        mapExpr(body)
      }
    }
   ...
   */
  def instrumentation(fd: FunDef, e: Expr, subInsts: Seq[Expr]) : Expr
  
  def instrumentIfThenElse(e: IfExpr, condInst: Expr, thenInst: Expr, elzeInst: Expr) : (Expr, Expr)

  class ExprInstrumenter(ctx: LeonContext, fd: FunDef, funMap: Map[FunDef, FunDef]) {
    ...
    def tupleifyRecur(e: Expr, subs: Seq[Expr], recons: Seq[Expr] => Expr,
      ...
      if (subs.size == 0) {
        ...
        e match {
          case t: Terminal => Tuple(Seq(recons(Seq()), instrumentation(fd, t, Seq())))
          case f @ FunctionInvocation(tfd, args) => {
           ...
           //Note we need to ensure that the last element of list
            ...
            val baseExpr = Tuple(Seq(valexpr, instrumentation(fd, e, subeInsts :+ instexpr)))
            Let(resvar.id, newFunInv, baseExpr)
          }

          case _ => {
            val exprPart = recons(subeVals)                        
            Tuple(Seq(exprPart, instrumentation(fd, e, subeInsts)))
          }
        }
      } else {
    ...
    def transform(e: Expr): Expr = e match {
      case Let(i, v, b) =>
        val ir = Variable(FreshIdentifier("ir", TupleType(Seq(v.getType, instrumentationType)), true))        
        val r = Variable(FreshIdentifier("r", TupleType(Seq(b.getType, instrumentationType)), true))                
        Let(ir.id, transform(v),
          Let(r.id, replace(Map[Expr,Expr](Variable(i) -> TupleSelect(ir, 1)), transform(b)),
            Tuple(Seq(TupleSelect(r, 1),                
                instrumentation(fd, e, Seq(TupleSelect(r, 2), TupleSelect(ir, 2)))))))
      ...
      case ife@IfExpr(cond, th, elze) => {
        ...
        val rescond = Variable(FreshIdentifier("c", 
            TupleType(Seq(cond.getType, instrumentationType)), true))                      
        val resthen = Variable(FreshIdentifier("t", 
            TupleType(Seq(th.getType, instrumentationType)), true))
        val reselse = Variable(FreshIdentifier("e", 
            TupleType(Seq(elze.getType, instrumentationType)), true))
